...
class AnimationItem(QToolButton):
    def __init__(self, name: str, frames: list[AnimationFrame], is_random: bool) -> None:
        ...
        for frame in frames:
            ...
            assert image.width() == oled.WIDTH, f"Image width expected {oled.WIDTH}px, got {image.width()}px"
            assert image.height() == oled.HEIGHT, f"Image height expected {oled.HEIGHT}px, got {image.height()}px"
